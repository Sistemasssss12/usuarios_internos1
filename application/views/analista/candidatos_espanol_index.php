  ...
  $('#tabla').DataTable({
      ...
      },
      {
        ...
        mRender: function(data, type, full) {
          var subcliente = (full.subcliente === null || full.subcliente === "") ? '' :
            '<span class="badge badge-pill badge-primary">Subcliente: ' + full.subcliente + '</span><br>';
          var analista = (full.usuario === null || full.usuario === '') ? 'Analista: Sin definir' :
            'Analista: ' + full.usuario;
          var reclutador = (full.reclutadorAspirante !== null) ?
            '<br><span class="badge badge-pill badge-info">Reclutador(a): ' + full.reclutadorAspirante +
            '</span>' : '';
          return '<span class="badge badge-pill badge-dark">#' + full.id +
            '</span><br><a data-toggle="tooltip" class="sin_vinculo" style="color:black;"><b>' + data +
            '</b></a><br>' + subcliente + '<span class="badge badge-pill badge-light">' + analista + '</span>' +
            reclutador;
        }
      },
      {
        ...
        mRender: function(data, type, full) {
          ...
          return fechas = '<b>Alta:</b> ' + fechaAlta + '<br>' + '<b>Formulario:</b> ' + fechaFormulario +
            '<br>' + '<b>Documentos:</b> ' + fechaDocumentos + '<br>' + '<b>Final:</b> ' + fechaFinal
        }
      },
      {
        title: 'SLA',
          ...
          if (full.cancelado == 0) {
            if (full.tipo_conclusion != 0) {
              ...
              if (full.ingles == 1) {
                ...
                detallesProceso =
                  '<div class="dropdown-divider"></div><a href="javascript:void(0);" class="dropdown-item" id="documentos_requeridos">Detalles del proceso del candidato</a>'
              }
          ...
          if (full.cancelado == 0) {
            if (full.socioeconomico == 1) {
              ...
              if (data != null) {
                ...
                if (full.id_cliente == 172 || full.id_cliente == 178 || full.id_cliente == 201 || full
                  .id_cliente == 205 || full.id_cliente == 211 || full.id_cliente == 235 || full.id_cliente ==
                  236 || full.id_cliente == 1 || full.id_cliente == 244 || full.id_cliente == 249 || full
                  .id_cliente == 250 || full.id_cliente == 251 || full.id_cliente == 2) {
                ...
                if (full.fecha_resultado != null && full.fecha_resultado != "") {
                  if (full.resultado_doping == 1) {
                    salida +=
                      ...
                      full.idDoping + '" value="' + full.idDoping + '"></form></div>';
                  } else {
                    salida +=
                      '<b>Doping: </b><div style="display: inline-block;margin-left:3px;"><form id="pdfForm' +
                      full.idDoping +
            ...
            if (full.medico == 1) {
              if (full.idMedico != null) {
                if (full.conclusion != null && full.descripcion != null) {
                  salida += '<b>Médico:</b> <div style="display: inline-flex;"><form id="formMedico' + full
                    .idMedico +
                    '" action="<?php echo base_url('Medico/crearPDF'); ?>" method="POST"><a href="javascript:void(0);" data-toggle="tooltip" title="Descargar documento final" id="pdfMedico" class="fa-tooltip icono_datatable icono_medico"><i class="fas fa-file-pdf"></i></a><input type="hidden" name="idMedico" id="idMedico' +
                    full.idMedico + '" value="' + full.idMedico + '"></form></div>';
                } else {
      ...
      },
      {
        ...
        mRender: function(data, type, full) {
          if (full.cancelado == 0 || full.socioeconomico == 0) {
            ...
            let previo = '';
            if (full.tipo_conclusion != 0) {
              if (full.id_cliente != 96 && full.id_cliente != 1 && full.id_cliente != 2 && full.id_cliente !=
                233) {
                if (full.status_bgc == 0) {
                  previo = (full.fecha_nacimiento != null && full.fecha_nacimiento != '0000-00-00') ?
                    ' <div style="display: inline-flex;"><form id="reportePrevioForm' + data +
                  ...
                  if (full.tipo_conclusion > 0) {
                    ...
                    return '<div style="display: inline-block;"><form id="reporteForm' + data +
                      '" action="<?php echo base_url('Candidato_Conclusion/createPDF'); ?>" method="POST"><a href="javascript:void(0);" data-toggle="tooltip" title="Descargar reporte PDF" id="reportePDF" class="fa-tooltip icono_datatable ' +
                      icono_resultado +
              ...
              if (full.id_cliente == 96) {
                ...
                if (full.status == 1 || full.status == 2) {
                  if (full.idBGC === null) {
                    ...
                    pdfSimple = '<div style="display: inline-block;"><form id="reporteFormSimple' + data +
                      '" action="<?php echo base_url('Subcliente_RTS/crearPDFSimple'); ?>" method="POST"><a href="javascript:void(0);" data-toggle="tooltip" title="Descargar documento final simple" id="simplePDF" class="fa-tooltip icono_datatable ' +
                      icono_resultado +
                      '"><i class="far fa-file-pdf"></i></a><input type="hidden" name="idCandidatoSimple" id="idCandidatoSimple' +
                      data + '" value="' + data + '"></form></div>';
                    pdfCompleto = '<div style="display: inline-block;"><form id="reporteFormCompleto' + data +
                      '" action="<?php echo base_url('Subcliente_RTS/crearPDFCompleto'); ?>" method="POST"><a href="javascript:void(0);" data-toggle="tooltip" title="Descargar documento final completo" id="completoPDF" class="fa-tooltip icono_datatable ' +
                      icono_resultado +
              ...
              if (full.id_cliente == 2 || full.id_cliente == 233) {
                let iconoChecks =
                  '<a href="javascript:void(0)" data-toggle="tooltip" title="Actualizacion del checklist del candidato" id="actualizar_checks" class="fa-tooltip icono_datatable"><i class="fas fa-tasks"></i>';
                let pdfPrevio = '<div style="display: inline-block;"><form id="reportePrevioForm' + data +
                  '" action="<?php echo base_url('Client/crearReportePDF'); ?>" method="POST"><a href="javascript:void(0);" data-toggle="tooltip" title="Descargar documento final" id="reportePrevioPDF" class="fa-tooltip icono_datatable"><i class="fas fa-file-powerpoint"></i></a><input type="hidden" name="idCandidatoPDF" id="idCandidatoPDF' +
                  data + '" value="' + data + '"></form></div>';
                if (full.status_bgc == 0) {
                  ...
                  if (full.id_cliente != 233) {
                    var pdf = '<div style="display: inline-block;"><form id="reporteForm' + data +
                      '" action="<?php echo base_url('Client/crearReportePDF'); ?>" method="POST"><a href="javascript:void(0);" data-toggle="tooltip" title="Descargar documento final" id="reportePDF" class="fa-tooltip icono_datatable"><i class="fas fa-file-pdf"></i></a><input type="hidden" name="idCandidatoPDF" id="idCandidatoPDF' +
                      data + '" value="' + data + '"></form></div>';
                    return icono + pdf;
                  }
                  if (full.id_cliente == 233) {
    ...
    rowCallback: function(row, data) {
      $('a#datos_generales', row).off('click').bind('click', () => {
        ...
        let grados_estudio_data = getDataCatalogo(url_grados_estudio, 'id', 0, idiomaCliente);
        ...
        let url_medio_transporte = '<?php echo base_url('Funciones/getMediosTransporte'); ?>';
        let transportes_data = getDataCatalogo(url_medio_transporte, 'nombre', 0, 'espanol');
        let url_tipo_identificacion = '<?php echo base_url('Funciones/getTiposIdentificacion'); ?>';
        let identificaciones_data = getDataCatalogo(url_tipo_identificacion, 'nombre', 0, 'espanol');
        $.ajax({
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  ...
                  if (dato[i]['tipo_etiqueta'] == 'select') {
                    $('#rowForm').append(dato[i]['titulo_seccion_modal'] + dato[i]['grid_col_inicio'] +
                      dato[i]['label'] + dato[i]['etiqueta_inicio'] + opciones + dato[i][
                        'etiqueta_cierre'
                  ...
                  if (dato[i]['tipo_etiqueta'] == 'input') {
                    $('#rowForm').append(dato[i]['titulo_seccion_modal'] + dato[i]['grid_col_inicio'] +
                      dato[i]['label'] + dato[i]['etiqueta_inicio'] + dato[i]['etiqueta_cierre'] +
                      dato[i]['grid_col_cierre']);
                    $('#' + dato[i]['atr_id']).val(valores[dato[i]['referencia']]);
                  }
                  ...
                  if (dato[i]['referencia'] == 'id_municipio') {
                    if (valores['id_municipio'] != null && valores['id_municipio'] != 0) {
                      $('#rowForm').append('<script>getMunicipios($("#estado").val(), "#municipio", ' +
                        valores["id_municipio"] + ');<\/script>');
                    } else {
                      $('#municipio').empty();
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  //* Get Data Catalogos
                  if (dato[i]['referencia'] == 'id_grado_estudio' || dato[i]['referencia'] ==
                    'id_tipo_studies')
                    opciones = grados_estudio_data;
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  ...
                  if (dato[i]['referencia'] == 'id_grado_estudio')
                    opciones = grados_estudio_data;
                  if (dato[i]['referencia'] == 'primaria_validada' || dato[i]['referencia'] ==
                    'secundaria_validada' || dato[i]['referencia'] == 'preparatoria_validada' || dato[i]
                    ['referencia'] == 'licenciatura_validada')
                  ...
                  if (data.id_estudios == 52) {
                    if (dato[i]['referencia'] == 'primaria_certificado' || dato[i]['referencia'] ==
                      'secundaria_certificado' || dato[i]['referencia'] == 'preparatoria_certificado' ||
                      dato[i]['referencia'] == 'licenciatura_certificado' || dato[i]['referencia'] ==
                      'actual_certificado')
                      opciones = '<option value="1">Sí</option><option value="0">No</option>';
                  }
                  if (data.id_estudios == 79) {
                    if (dato[i]['referencia'] == 'primaria_certificado' || dato[i]['referencia'] ==
                      'secundaria_certificado' || dato[i]['referencia'] == 'preparatoria_certificado' ||
                      dato[i]['referencia'] == 'licenciatura_certificado')
                      opciones =
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  ...
                  if (dato[i]['referencia'] == 'bebidas' || dato[i]['referencia'] == 'fumar' || dato[i][
                      'referencia'
                    ] == 'sindical' || dato[i]['referencia'] == 'partido' || dato[i]['referencia'] ==
                    'club' || dato[i]['referencia'] == 'tiene_familiar_penal' || dato[i][
                    'referencia'] == 'tiene_persona_empresa' || dato[i]['referencia'] ==
                    'tiene_familiar_policia' || dato[i]['referencia'] == 'tiene_otro_trabajo')
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  ...
                  if (dato[i]['referencia'] == 'tipo_sangre')
                    opciones = sanguineo_data;
                  if (dato[i]['referencia'] == 'tabaco_frecuencia' || dato[i]['referencia'] ==
                    'alcohol_frecuencia' || dato[i]['referencia'] == 'droga_frecuencia')
                    opciones = frecuencias_data;
                  if (dato[i]['referencia'] == 'practica_deporte')
                    opciones = '<option value="0">No</option><option value="1">Sí</option>';
                  if (dato[i]['referencia'] == 'tabaco' || dato[i]['referencia'] == 'alcohol' || dato[i]
                    ['referencia'] == 'droga')
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (var num = 1; num <= data.cantidad_ref_personales; num++) {
                $('#rowForm').append(
                  '<div class="alert alert-secondary btn-block text-center"><h5>Referencia #' + num +
                  '</h5></div>');
                //autor_anterior = '';
                for (let tag of dato) {
                  ...
                  // }
                  //* Get Data Catalogos
                  if (tag['referencia'] == 'sabe_trabajo' || tag['referencia'] == 'sabe_vive' || tag[
                  ...
                  if (tag['tipo_etiqueta'] == 'select') {
                    $('#rowForm').append(tag['grid_col_inicio'] + tag['label'] + tag[
                      'etiqueta_inicio'] + opciones + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
                  }
                  if (tag['tipo_etiqueta'] == 'input') {
                    $('#rowForm').append(tag['titulo_seccion_modal'] + tag['grid_col_inicio'] + tag[
                      'label'] + tag['etiqueta_inicio'] + tag['etiqueta_cierre'] + tag[
                      'grid_col_cierre']);
                  }
                  if (tag['tipo_etiqueta'] == 'textarea') {
                    $('#rowForm').append(tag['grid_col_inicio'] + tag['label'] + tag[
                      'etiqueta_inicio'] + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
                  }
                }
                //* Boton Guardar
                $('#rowForm').append(
                  '<div class="col-6"><button type="button" id="btnGuardarRefPersonal' + num +
                  '" class="btn btn-success btn-block mt-3 mb-5" >Guardar la Referencia #' + num +
                ...
                $('#btnGuardarRefPersonal' + num).attr("onclick", "submitForm(" + JSON.stringify(
                  formNewParams) + ")");
              }
              //* Values
              if (valores != 0) {
                ...
                for (let valor of valores) {
                  ...
                  $('#titleForm').html('Referencias personales del candidato: <br>' + data.candidato)
                  $('#btnGuardarRefPersonal' + (index + 1)).attr("onclick", "submitForm(" + JSON
                    .stringify(formParams) + ")");
                  $('#btnEliminarRefPersonal' + (index + 1)).attr("onclick",
                    "mostrarMensajeConfirmacion(\"eliminar referencia personal\"," + (index + 1) +
                    "," + idRefPersonal + ")");
                  //$('#btnSubmitForm').attr("onclick","submitForm("+JSON.stringify(formParams)+")");
                  //$("#formModal").modal('show');
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (var num = 1; num <= data.cantidad_ref_profesionales; num++) {
                ...
                //* Boton Guardar
                $('#rowForm').append(
                  '<div class="col-6"><button type="button" id="btnGuardarRefProfesional' + num +
                  '" class="btn btn-success btn-block mt-3 mb-5" >Guardar la Referencia #' + num +
                ...
                $('#btnGuardarRefProfesional' + num).attr("onclick", "submitForm(" + JSON.stringify(
                  formNewParams) + ")");
              }
              //* Values
              if (valores != 0) {
                ...
                for (let valor of valores) {
                  ...
                  $('#titleForm').html('Referencias profesionales del candidato: <br>' + data.candidato)
                  $('#btnGuardarRefProfesional' + (index + 1)).attr("onclick", "submitForm(" + JSON
                    .stringify(formParams) + ")");
                  $('#btnEliminarRefProfesional' + (index + 1)).attr("onclick",
                    "mostrarMensajeConfirmacion(\"eliminar referencia profesional\"," + (index + 1) +
                    "," + idRefProfesional + ")");
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (var num = 1; num <= data.cantidad_ref_academicas; num++) {
                //* HTML
                $('#rowForm').append(
                  '<div class="alert alert-warning btn-block text-center"><h5>Referencia #' + num +
                  '</h5></div>');
                for (let tag of dato) {
                  if (tag['tipo_etiqueta'] == 'select') {
                    $('#rowForm').append(tag['grid_col_inicio'] + tag['label'] + tag[
                ...
                //* Boton Guardar
                $('#rowForm').append(
                  '<div class="col-6"><button type="button" id="btnGuardarRefAcademica' + num +
                  '" class="btn btn-success btn-block mt-3 mb-5" >Guardar la Referencia #' + num +
                ...
                $('#btnGuardarRefAcademica' + num).attr("onclick", "submitForm(" + JSON.stringify(
                  formNewParams) + ")");
              }
              //* Values
              if (valores != 0) {
                ...
                for (let valor of valores) {
                  ...
                  $('#titleForm').html('Referencias académicas del candidato: <br>' + data.candidato)
                  $('#btnGuardarRefAcademica' + (index + 1)).attr("onclick", "submitForm(" + JSON
                    .stringify(formParams) + ")");
                  $('#btnEliminarRefAcademica' + (index + 1)).attr("onclick",
                    "mostrarMensajeConfirmacion(\"eliminar referencia academica\"," + (index + 1) +
                    "," + idRefAcademica + ")");
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (var num = 1; num <= data.cantidad_ref_vecinales; num++) {
                ...
                //* Boton Guardar
                $('#rowForm').append(
                  '<div class="col-6"><button type="button" id="btnGuardarRefVecinal' + num +
                  '" class="btn btn-success btn-block mt-3 mb-5" >Guardar la Referencia #' + num +
                ...
                $('#btnGuardarRefVecinal' + num).attr("onclick", "submitForm(" + JSON.stringify(
                  formNewParams) + ")");
              }
              //* Values
              if (valores != 0) {
                ...
                for (let valor of valores) {
                  ...
                  $('#titleForm').html('Referencias vecinales del candidato: <br>' + data.candidato)
                  $('#btnGuardarRefVecinal' + (index + 1)).attr("onclick", "submitForm(" + JSON
                    .stringify(formParams) + ")");
                  $('#btnEliminarRefVecinal' + (index + 1)).attr("onclick",
                    "mostrarMensajeConfirmacion(\"eliminar referencia vecinal\"," + (index + 1) +
                    "," + idRefVecinal + ")");
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (var num = 1; num <= data.cantidad_ref_clientes; num++) {
                ...
                //* Boton Guardar
                $('#rowForm').append(
                  '<div class="col-6"><button type="button" id="btnGuardarRefCliente' + num +
                  '" class="btn btn-success btn-block mt-3 mb-5" >Guardar la Referencia #' + num +
                ...
                $('#btnGuardarRefCliente' + num).attr("onclick", "submitForm(" + JSON.stringify(
                  formNewParams) + ")");
              }
              //* Values
              if (valores != 0) {
                ...
                for (let valor of valores) {
                  ...
                  $('#titleForm').html('Referencias de clientes del candidato: <br>' + data.candidato)
                  $('#btnGuardarRefCliente' + (index + 1)).attr("onclick", "submitForm(" + JSON
                    .stringify(formParams) + ")");
                  $('#btnEliminarRefCliente' + (index + 1)).attr("onclick",
                    "mostrarMensajeConfirmacion(\"eliminar referencia cliente\"," + (index + 1) +
                    "," + idRefCliente + ")");
                  index++;
                }
              }
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              $('#formModal .modal-footer').prepend(
                ...
                url_files + '\',' + data.id + ',\'Hola\')">Ver archivos del candidato</button>')
              $('#formModal .modal-body').addClass('escrolable');
            } else {
              $('#rowForm').html(
                '<div class="col-12 text-center"><h5><b>Formulario no registrado para este candidato</b></h5></div>'
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  //* Get Data Catalogos
                  if (dato[i]['referencia'] == 'adeudo_muebles' || dato[i]['referencia'] ==
                    'tiene_credito_banco' || dato[i]['referencia'] == 'unico_solvente')
                    opciones = '<option value="0">No</option><option value="1">Sí</option>';
                  //* HTML
                  if (dato[i]['tipo_etiqueta'] == 'select') {
                    $('#rowForm').append(dato[i]['titulo_seccion_modal'] + dato[i]['grid_col_inicio'] +
      ...
      $('a#datos_vivienda', row).off('click').bind('click', () => {
        ...
        let url_condiciones_vivienda = '<?php echo base_url('Funciones/getCondicionesVivienda'); ?>';
        let condiciones_vivienda_data = getDataCatalogo(url_condiciones_vivienda, 'id', 0, 'espanol');
        $.ajax({
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  ...
                  if (dato[i]['referencia'] == 'calidad_mobiliario' || dato[i]['referencia'] ==
                  ...
                  if (dato[i]['referencia'] == 'sala' || dato[i]['referencia'] == 'comedor' || dato[i][
                      'referencia'
                    ] == 'cocina' || dato[i]['referencia'] == 'patio' || dato[i]['referencia'] ==
                    'cochera')
                  ...
                  if (dato[i]['tipo_etiqueta'] == 'select') {
                    $('#rowForm').append(dato[i]['grid_col_inicio'] + dato[i]['label'] + dato[i][
                      'etiqueta_inicio'
                    ] + opciones + dato[i]['etiqueta_cierre'] + dato[i]['grid_col_cierre']);
                    $('#' + dato[i]['atr_id']).val(valores[dato[i]['referencia']]);
                  }
                  if (dato[i]['tipo_etiqueta'] == 'textarea') {
                    $('#rowForm').append(dato[i]['grid_col_inicio'] + dato[i]['label'] + dato[i][
                      'etiqueta_inicio'
                    ] + dato[i]['etiqueta_cierre'] + dato[i]['grid_col_cierre']);
                    $('#' + dato[i]['atr_id']).val(valores[dato[i]['referencia']]);
                  }
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  //* Get Data Catalogos
                  if (dato[i]['referencia'] == 'agua' || dato[i]['referencia'] == 'drenaje' || dato[i][
                      'referencia'
                    ] == 'electricidad' || dato[i]['referencia'] == 'alumbrado' || dato[i][
                    'referencia'] == 'iglesia' || dato[i]['referencia'] == 'transporte' || dato[i][
                      'referencia'
                    ] == 'hospital' || dato[i]['referencia'] == 'policia' || dato[i]['referencia'] ==
                    'mercado' || dato[i]['referencia'] == 'plaza_comercial' || dato[i]['referencia'] ==
                    'aseo_publico' || dato[i]['referencia'] == 'areas_verdes')
                    opciones =
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  //* Get Data Catalogos
                  if (dato[i]['referencia'] == 'resultado_oig' || dato[i]['referencia'] ==
                    'resultado_sam')
          ...
          success: function(res) {
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  //* Get Data Catalogos
                  if (dato[i]['referencia'] == 'resultado_ofac' || dato[i]['referencia'] ==
                    'resultado_oig' || dato[i]['referencia'] == 'resultado_sam' || dato[i][
                    'referencia'] == 'res_data_juridica' || dato[i]['referencia'] ==
      ...
      $('a#documentos_requeridos', row).bind('click', () => {
        $.ajax({
          ...
          success: function(res) {
            ...
            $('#mensaje').html(
              '<div class="alert alert-info btn-block">Los siguientes documentos son los requeridos para el proyecto del candidato.</div>' +
              res);
      ...
      $("a#final", row).bind('click', () => {
        ...
        if (data.tipo_conclusion == 1 || data.tipo_conclusion == 2 || data.tipo_conclusion == 3 || data
          .tipo_conclusion == 4 || data.tipo_conclusion == 5 || data.tipo_conclusion == 6 || data
          .tipo_conclusion == 7 || data.tipo_conclusion == 10 || data.tipo_conclusion == 14 || data
          .tipo_conclusion == 15 || data.tipo_conclusion == 17 || data.tipo_conclusion == 19 || data
          .tipo_conclusion == 21) {
          $('.loader').css("display", "block");
          //* Datos generales
          ...
          if (data_social != 0) {
            ...
            if (social.religion != "" && social.religion != "Ninguna" && social.religion != "NINGUNA" &&
              social.religion != "No" && social.religion != "NO" && social.religion != "NA" && social
              .religion != "N/A" && social.religion != "No aplica" && social.religion != "NO APLICA" && social
              .religion != "No Aplica") {
              var religion = "profesa la religion " + social.religion + ".";
            } else {
              var religion = "no profesa alguna religión.";
            }
            if (social.cirugia != "" && social.cirugia != "Ninguna" && social.cirugia != "NINGUNA" && social
              .cirugia != "No" && social.cirugia != "NO" && social.cirugia != "NA" && social.cirugia !=
              "N/A" && social.cirugia != "No aplica" && social.cirugia != "NO APLICA" && social.cirugia !=
              "No Aplica" && social.cirugia != "0") {
              var cirugia = "Cuenta con cirugia(s) de " + social.cirugia + ".";
            } else {
              var cirugia = "No cuenta con cirugias.";
            }
            if (social.enfermedades != "" && social.enfermedades != "Ninguna" && social.enfermedades !=
              "NINGUNA" && social.enfermedades != "No" && social.enfermedades != "NO" && social
              .enfermedades != "NA" && social.enfermedades != "N/A" && social.enfermedades != "No aplica" &&
              social.enfermedades != "NO APLICA" && social.enfermedades != "No Aplica" && social
              .enfermedades != "0") {
              var enfermedades = "Tiene alguna(s) enfermedad(es) con antecedente familiar como " + social
                .enfermedades + ".";
            } else {
          ...
          if (data_salud != 0) {
            ...
            if (salud.enfermedad_cronica == 'No aplica' || salud.enfermedad_cronica == 'NA' || salud
              .enfermedad_cronica == 'N/A' || salud.enfermedad_cronica == 'No padece' || salud
              .enfermedad_cronica == 'No tiene') {
              var enfermedad_cronica = 'no padece enfermedades crónicas,';
            } else {
              var enfermedad_cronica = 'padece de ' + salud.enfermedad_cronica + ',';
            }
            if (salud.accidentes == 'No aplica' || salud.accidentes == 'NA' || salud.accidentes == 'N/A' ||
              salud.accidentes == 'No ha tenido') {
              var accidente = 'no ha sufrido accidentes graves,';
            } else {
              var accidente = 'ha sufrido de ' + salud.accidentes + ',';
            }
            if (salud.alergias == 'No aplica' || salud.alergias == 'NA' || salud.alergias == 'N/A' || salud
              .alergias == 'No ha tenido' || salud.alergias == 'No tiene') {
              var alergias = 'no reporta alergias.';
            } else {
              var alergias = 'reporta alergias de ' + salud.alergias + '.';
            }
            if (salud.tabaco == 'SI') {
              var salud_tabaco = 'Refiere consumir tabaco con una frecuencia de ' + salud.tabaco_frecuencia
                .toLowerCase();
            } else {
              var salud_tabaco = 'Niega la ingesta de tabaco';
            }
            if (salud.droga == 'SI') {
              var salud_droga = ' , hace uso de droga con una frecuencia de ' + salud.droga_frecuencia
                .toLowerCase();
            } else {
              var salud_droga = ' , no hace uso de droga';
            }
            if (salud.alcohol == 'SI') {
              var salud_alcohol = ' y refiere consumir alcohol de manera ' + salud.alcohol_frecuencia
                .toLowerCase();
            } else {
              var salud_alcohol = ' y no consume alcohol';
            }
            if (salud.practica_deporte == 1) {
              var practica_deporte = 'Como actividad física menciona practicar ' + salud.practica_deporte +
                ' ' + salud.deporte_frecuencia;
            } else {
          ...
          if (data.tipo_conclusion == 21) {
            ...
            $('#comentario_bgc').val("");
            let sumaGastos = (parseInt(economia.renta) + parseInt(economia.alimentos) + parseInt(economia
              .servicios) + parseInt(economia.transporte) + parseInt(economia.otros));

            $("#personal1").val("C. " + data.candidato + ", de " + data.edad + " años de edad, es " + data
              .estado_civil + ", reside en " + data.municipio + ", " + data.estado + ".");
            ...
            $("#socio2").val("Los ingresos mensuales son de $" + economia.sueldo +
              ...
              ". Con respecto a si posee bienes el candidato menciona que " + economia.bienes +
          ...
          if (data.tipo_conclusion == 14) {
            ...
            $("#personal1").val("C. " + data.candidato + ", de " + data.edad + " años de edad, es " + data
              .estado_civil + ", reside en " + data.municipio + ", " + data.estado + " " + religion +
              ". Cuenta con estudios en " + data.grado);
            $("#personal2").val("Menciona " + bebidas + " bebidas alcohólicas y " + fuma + "." + cirugia +
              ". Su plan a corto plazo es " + social.corto_plazo + "; y su meta a mediano plazo es " +
              social.mediano_plazo + ".");
            $("#personal3").val("Sus referencias personales lo describen como " + refs_comentarios + ".");
            $("#socio1").val("Actualmente vive en un/una " + vivienda.vivienda + ", en una zona " + vivienda
              .zona + "; el mobiliario en su interior se observa de " + calidad + ", la vivienda es " +
              tamano + ", en condiciones " + vivienda.condiciones + ". " + servicios.basicos + "; " +
              servicios.vias_acceso + " y " + servicios.rutas_transporte + ".");
            $("#socio2").val("El(La) candidato(a) vive " + personas_mismo_domicilio +
              ". Los ingresos mensuales ______; gastan mensualmente $" + economia.otros +
              ". La vivienda cuenta con " + distribucion_hogar + ".");
            $("#laboral2").val(comentarios_laborales);
            $("#visita2").val("De acuerdo a su(s) referencia(s) vecinal(es), describen que es " + vecinales);
          ...
          if (data.tipo_conclusion == 1) {
            ...
            $('#conclusion_investigacion').val('')
            $("#personal1").val(data.candidato + ", de " + data.edad + " años, reside en " + data.municipio +
              ", " + data.estado + ". Es " + data.estado_civil + " y " + religion);
            $("#personal2").val("Refiere " + bebidas + " bebidas alcohólicas. " + fuma + " " + cirugia + " " +
              enfermedades + " Sus referencias personales lo describen como " + refs_comentarios + ".");
            ...
            $("#socio1").val("Actualmente vive en un/una " + vivienda.vivienda +
              ...
              ". El nivel de la zona es " + vivienda.zona + ", el mobiliario es de calidad " + calidad +
              ", la vivienda es " + tamano + " y en condiciones " + vivienda.condiciones +
              ". La distribución de su " + vivienda.vivienda + " es " + vivienda.distribucion);
            $("#socio2").val(data.candidato + " declara en sus ingresos " + data.ingresos +
              ". Los gastos generados en el hogar son solventados por _____. Cuenta con " + data.muebles +
              " " + adeudo + ".");
            $("#laboral1").val("Señaló " + trabajos + " referencias laborales");
            $("#laboral2").val(comentarios_laborales);
            $("#visita1").val("El candidato durante la visita: " + data.comentarioVisitador);
            $("#visita2").val("De acuerdo a la referencia vecinal, el candidato es considerado: " +
            vecinales);
            $("#completarModal").modal('show');
          }
          if (data.tipo_conclusion == 2) {
            ...
            $('#conclusion_investigacion').val('')
            if (data.visitador == 1) {
              $("#personal1").val("Se aplicó estudio socioeconómico a " + data.candidato + ", de " + data
                .edad + " años, originario de " + originario + " con CURP:" + data.curp + " y NSS:" + data
                .nss + "; estado civil " + data.estado_civil.toLowerCase() + ", vive " +
                personas_mismo_domicilio + "en el domicilio " + data.calle + " #" + data.exterior + " " +
                data.interior + ", colonia " + data.colonia + ", desde hace " + data.tiempo_dom_actual +
                ", en una propiedad " + propiedad + " que se encuentra ubicada en una zona " + vivienda
                .tipo_zona.toLowerCase() + " de clase " + vivienda.zona.toLowerCase() + ".");
            } else {
              $("#personal1").val("No se pudo aplicar el estudio socioeconómico.");
            }
            $("#personal2").val("En cuanto a su salud " + enfermedad_cronica + " " + accidente +
              " su tipo de sangre es " + salud.tipo_sangre + " y " + alergias + " " + salud_tabaco +
              salud_droga + salud_alcohol + ". " + practica_deporte + ".");
            $("#personal4").val("Sus estudios máximos son de " + maximo_estudio +
              " su experiencia laboral es como " + historial_puestos + ".");
            $("#socio1").val("Referente a su economía, menciona solventar sus gastos con " + economia
              .observacion + ".");
          ...
          if (data.tipo_conclusion == 4) {
            ...
            $('.es_conclusion').val('')
            $("#personal1").val(data.candidato + ", de " + data.edad + " años, es originario de " +
              originario + ". Tiene como máximo grado de estudios: " + data.grado + ". Refiere ser " +
              social.religion + ". Su plan a corto plazo: " + social.corto_plazo +
          ...
          if (data.tipo_conclusion == 5) {
            ...
            $("#personal1").val(data.candidato + ", de " + data.edad + " años, de nacionalidad " + data
              .nacionalidad + ", que reside en " + data.municipio + ", " + data.estado + ". Es " + data
              .estado_civil);
            $("#personal2").val("Refiere " + bebidas + " bebidas alcohólicas. " + fuma + " " + cirugia + " " +
              enfermedades + " Sus referencias personales lo describen como " + refs_comentarios + ".");
          ...
          if (data.tipo_conclusion == 6) {
            ...
            $('#conclusion_investigacion').prop('disabled', true)
            $("#personal1").val(data.candidato + ", de " + data.edad + " años, de nacionalidad " + data
              .nacionalidad + ". Es " + data.estado_civil);
            $("#personal2").val(
          ...
          if (data.tipo_conclusion == 7) {
            ...
            $("#personal1").val(data.candidato + ", de " + data.edad + " años, es originario de " +
              ...
              social.religion + ". Su plan a corto plazo: " + social.corto_plazo +
              ". Su plan a mediano plazo: " + social.mediano_plazo);
            $("#personal2").val("Refiere " + bebidas + " bebidas alcohólicas " + social.bebidas_frecuencia +
              ". " + fuma + " Sus referencias personales lo describen como " + refs_comentarios + ".");
            $("#laboral1").val("Señaló " + trabajos + " referencias laborales.");
            $("#laboral2").val(" es quien nos valida referencia laboral.");
            $("#socio1").val(data.candidato + ", actualmente vive en un/una " + vivienda.vivienda +
              ", con un tiempo de residencia de " + vivienda.tiempo_residencia +
              ". El nivel de la zona es " + vivienda.zona + ", el mobiliario es de calidad " + calidad +
              ", la vivienda es " + tamano + " y en condiciones " + vivienda.condiciones);
            $("#socio2").val(
              "Los gastos generados en el hogar son solventados por _____. Sus referencias vecinales describen que es " +
              vecinales + ". El candidato cuenta con " + data.muebles + " " + adeudo);
          ...
          if (data.tipo_conclusion == 10) {
            ...
            $("#socio2").prop('disabled', false)
            $("#personal1").val(data.candidato + ", de " + data.edad + " años, es originario de " +
              originario + ". Tiene como máximo grado de estudios: " + data.grado + ". Es " + data
              .estado_civil + " y " + religion);
            $("#personal2").val("Refiere " + bebidas + " bebidas alcohólicas " + social.bebidas_frecuencia +
              ". " + fuma + ". Refiere ser " + social.religion + ". Su plan a corto plazo: " + social
              .corto_plazo + ". Su plan a mediano plazo: " + social.mediano_plazo);
            $("#laboral2").val("Indica que tiene trabajando para " + data.cliente + " por " + extra_laboral
              .actual_activo);
            $("#socio1").val(data.candidato + ", actualmente vive en un/una " + vivienda.vivienda +
              ", con un tiempo de residencia de " + vivienda.tiempo_residencia +
              ". El nivel de la zona es " + vivienda.zona + ", el mobiliario es de calidad " + calidad +
        ...
        $.ajax({
          ...
          success: function(res) {
            ...
            if (res != null) {
              ...
              var observaciones = (dato['comentarios'] != null) ? '- ' + dato['comentarios'] :
                'Las observaciones no han sido ( o no fueron) registradas';
              $('#tituloObservaciones').html('La visita registrada al candidato: <b><span>' +
                candidato + '</span></b>, tuvo las siguientes observaciones: ');
            ...
            if (res != 0) {
              ...
              for (let i = 0; i < dato.length; i++) {
                if (valores != 0) {
                  ...
                  //* HTML
                  if (dato[i]['tipo_etiqueta'] == 'input') {
                    $('#rowExtralaboral').append(dato[i]['titulo_seccion_modal'] + dato[i][
                        'grid_col_inicio'
                      ] + dato[i]['label'] + dato[i]['etiqueta_inicio'] + dato[i]['etiqueta_cierre'] +
                      dato[i]['grid_col_cierre']);
                    $('#' + dato[i]['atr_id']).val(valores[dato[i]['referencia']]);
                  ...
                  if (dato[i]['tipo_etiqueta'] == 'select') {
                    $('#rowExtralaboral').append(dato[i]['grid_col_inicio'] + dato[i]['label'] + dato[i]
                      ['etiqueta_inicio'] + opciones + dato[i]['etiqueta_cierre'] + dato[i][
                        'grid_col_cierre'
                      ]);
                    $('#' + dato[i]['atr_id']).val(valores[dato[i]['referencia']]);
                  }
                  if (dato[i]['tipo_etiqueta'] == 'textarea') {
                    $('#rowExtralaboral').append(dato[i]['grid_col_inicio'] + dato[i]['label'] + dato[i]
                      ['etiqueta_inicio'] + dato[i]['etiqueta_cierre'] + dato[i]['grid_col_cierre']);
                    $('#' + dato[i]['atr_id']).val(valores[dato[i]['referencia']]);
                  }
                } else {
                  //* Get Data Catalogos
      ...
      $("a#psicometria", row).bind('click', () => {
        $('#subirArchivoModal #titulo_modal').html('Carga de archivo de psicometría del candidato: <br>' +
          data.candidato);
      ...
      $("a#beca", row).bind('click', () => {
        $('#subirArchivoModal #titulo_modal').html(
          'Carga de archivo de estudio para solicitud de beca del candidato: <br>' + data.candidato);
      ...
      if (res != 0) {
        ...
        for (let number = 0; number < valores.length; number++) {
          $('#rowFamiliares').append(
            '<div class="alert alert-info btn-block"><h5 class="text-center">Familiar #' + totalFamiliares +
            '</h5></div><br>');
          //for(let i = 0; i < dato.length; i++){
          for (let tag of dato) {
            ...
            if (tag['tipo_etiqueta'] == 'select') {
              $('#rowFamiliares').append(tag['grid_col_inicio'] + tag['label'] + tag['etiqueta_inicio'] +
                opciones + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
            }
            if (tag['tipo_etiqueta'] == 'input') {
              $('#rowFamiliares').append(tag['titulo_seccion_modal'] + tag['grid_col_inicio'] + tag['label'] +
                tag['etiqueta_inicio'] + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
            }
            if (tag['tipo_etiqueta'] == 'textarea') {
              $('#rowFamiliares').append(tag['grid_col_inicio'] + tag['label'] + tag['etiqueta_inicio'] + tag[
          ...
          $('#rowFamiliares').append(
            ...
            valores[number]['id'] + ', \'' + valores[number]['nombre'] + '\')">Eliminar Integrante #' +
      ...
      setTimeout(function() {
        ...
        if (res != 0) {
          ...
          for (let tag of dato) {
            ...
            if (tag['tipo_etiqueta'] == 'select') {
              $('#rowNuevoFamiliar').append(tag['grid_col_inicio'] + tag['label'] + tag['etiqueta_inicio'] +
                opciones + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
            }
            if (tag['tipo_etiqueta'] == 'input') {
              $('#rowNuevoFamiliar').append(tag['titulo_seccion_modal'] + tag['grid_col_inicio'] + tag[
                'label'] + tag['etiqueta_inicio'] + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
            }
            if (tag['tipo_etiqueta'] == 'textarea') {
              $('#rowNuevoFamiliar').append(tag['grid_col_inicio'] + tag['label'] + tag['etiqueta_inicio'] +
          ...
          $('#rowNuevoFamiliar').append(
            ...
            id_candidato + ')"><i class="fas fa-plus-circle"></i> Registrar</a></div></div>');
        } else {
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      if (res != 0) {
        ...
        for (let number = 0; number < valores.length; number++) {
          $('#rowForm').append('<div class="alert alert-info btn-block"><h5 class="text-center">Address #' +
          ...
          for (let tag of dato) {
            ...
            if (tag['tipo_etiqueta'] == 'select') {
              $('#rowForm').append(tag['grid_col_inicio'] + tag['label_ingles'] + tag['etiqueta_inicio'] +
                opciones + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
            }
            if (tag['tipo_etiqueta'] == 'input') {
              $('#rowForm').append(tag['titulo_seccion_modal'] + tag['grid_col_inicio'] + tag['label_ingles'] +
                tag['etiqueta_inicio'] + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
            }
            if (tag['tipo_etiqueta'] == 'textarea') {
              $('#rowForm').append(tag['grid_col_inicio'] + tag['label_ingles'] + tag['etiqueta_inicio'] + tag[
                'etiqueta_cierre'] + tag['grid_col_cierre']);
            }
          }
          //* Boton Guardar
          ...
          $('#rowForm').append(
            ...
            valores[number]['id'] + ',' + number + ',' + valores[number]['id_candidato'] + ',' + id_seccion +
            ')">Update Address #' + totalDomicilios + '</a></div>');
            ...
            for (let tag of dato) {
              ...
              if (tag['referencia'] == 'id_estado') {
                ...
                $('#rowForm').append('<script>$("#id_estado' + index +
                  '").change(function(){getMunicipios($("#id_estado' + index + '").val(), "#id_municipio' +
                  index + '", "")})<\/script>');
              }
              if (tag['referencia'] == 'id_municipio') {
                ...
                if (valor['id_municipio'] != null && valor['id_municipio'] != 0) {
                  $('#rowForm').append('<script>getMunicipios(' + valor['id_estado'] + ', "#id_municipio' +
                    index + '", ' + valor['id_municipio'] + ');<\/script>');
                } else {
      ...
      setTimeout(function() {
        ...
        if (res != 0) {
          ...
          for (let tag of dato) {
            ...
            if (tag['tipo_etiqueta'] == 'select') {
              $('#rowNuevoItemForm').append(tag['grid_col_inicio'] + tag['label'] + tag['etiqueta_inicio'] +
                opciones + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
            }
            if (tag['tipo_etiqueta'] == 'input') {
              $('#rowNuevoItemForm').append(tag['titulo_seccion_modal'] + tag['grid_col_inicio'] + tag[
                'label'] + tag['etiqueta_inicio'] + tag['etiqueta_cierre'] + tag['grid_col_cierre']);
            }
            if (tag['tipo_etiqueta'] == 'textarea') {
              $('#rowNuevoItemForm').append(tag['grid_col_inicio'] + tag['label'] + tag['etiqueta_inicio'] +
          ...
          if (res != 0) {
            ...
            for (let i = 1; i <= 30; i++) {
              menu += '<li id="itemMenu' + i +
                '" data-toggle="tooltip" data-placement="left" title="Ir a la Laboral #' + i +
            ...
            for (let number = 0; number < 30; number++) {
              $('#rowHistorialLaboral').append(
                '<div class="col-12 mt-5"><div class="text-center" id="topLaboral' + (number + 1) +
                '"><h3 class="text-primary"><strong>Laboral #' + (number + 1) + '</strong></h3><br></div></div>'
                );
              for (let tag of dato) {
                //* Boton por autor del registro del campo
                if (autor_anterior != '' && tag['autor'] != autor_anterior) {
                  ...
                  $('#rowHistorialLaboral').append(
                    ...
                    (number + 1) + '" class="btn btn-success btn-block" onclick="guardarLaboral(0,' + number +
                    ',' + id + ',\'candidato\',\'' + candidato + '\')">Guardar Laboral #' + (number + 1) +
                    '</button></div><div class="col-6 mt-3 mb-5"><button type="button" id="btnEliminarLaboral' +
                    (number + 1) + '" class="btn btn-danger btn-block" disabled>Eliminar Laboral #' + (number +
                      1) + '</button></div>');
                  autor_anterior = tag['autor'];
                }
                if (autor_anterior == '' || tag['autor'] == autor_anterior) {
                ...
                if (id_seccion == 16 || id_seccion == 59) {
                  //* Opciones Select
                  if (tag['referencia'] == 'responsabilidad' || tag['referencia'] == 'iniciativa' || tag[
                      'referencia'] == 'eficiencia' || tag['referencia'] == 'disciplina' || tag['referencia'] ==
                    'puntualidad' || tag['referencia'] == 'limpieza' || tag['referencia'] == 'estabilidad' || tag[
                      'referencia'] == 'emocional' || tag['referencia'] == 'honestidad' || tag['referencia'] ==
                    'rendimiento' || tag['referencia'] == 'actitud') {
                    opciones =
                ...
                //* HTML
                if (tag['tipo_etiqueta'] == 'select') {
                  $('#rowHistorialLaboral').append(tag['titulo_seccion_modal'] + tag['grid_col_inicio'] + tag[
                    'label'] + tag['etiqueta_inicio'] + opciones + tag['etiqueta_cierre'] + tag[
                    'grid_col_cierre']);
                }
                if (tag['tipo_etiqueta'] == 'input') {
                  $('#rowHistorialLaboral').append(tag['titulo_seccion_modal'] + tag['grid_col_inicio'] + tag[
                ...
                if (id_seccion == 16 || id_seccion == 59) {
                  ...
                  if (tag['referencia'] == 'demanda' && tag['autor'] == 'analista') {
                    ...
                    $('#rowHistorialLaboral').append('<script>$("#aplicar_todo' + (number + 1) +
                      '").change(function(){var valor = $(this).val();$(".esAplicable' + (number + 1) +
                      '").val(valor)});<\/script>');
                  }
                }
              }
              ...
              $('#rowHistorialLaboral').append(
                ...
                (number + 1) + '" class="btn btn-success btn-block" onclick="guardarLaboral(0,' + number + ',' +
                id + ',\'analista\',\'' + candidato + '\')">Guardar Verificación #' + (number + 1) +
                '</button></div><div class="col-6 mt-3 mb-5"><button type="button" id="btnEliminarVerificacion' +
                (number + 1) + '" class="btn btn-danger btn-block" disabled>Eliminar Verificacion #' + (number +
                  1) + '</button></div>');
            ...
            if (valores != 0) {
              ...
              for (let valor of valores) {
                ...
                $('#btnGuardarLaboral' + (index + 1)).attr("onclick", "guardarLaboral(" + idLaboral + "," +
                  index + "," + id + ",\"candidato\",\"" + candidato + "\")");
                $('#btnEliminarLaboral' + (index + 1)).removeAttr('disabled');
                $('#btnEliminarLaboral' + (index + 1)).attr("onclick",
                  "mostrarMensajeConfirmacion(\"eliminar laboral\"," + idLaboral + "," + index + ")");
                index++;
              }
            }
              ...
              for (let valor2 of valores2) {
                ...
                for (let tag of dato) {
                  if (tag['autor'] == 'analista')
                    $('[name="' + tag['atr_id'] + '[]"]').eq((valor2['numero_referencia'] - 1)).val(valor2[tag[
                      'referencia']]);
                  if (flag == 0) {
                ...
                $('#btnGuardarVerificacion' + fila).attr("onclick", "guardarLaboral(" + idVerificacion + "," +
                  indiceInput + "," + id + ",\"analista\",\"" + candidato + "\")");
                ...
                $('#btnEliminarVerificacion' + valor2['numero_referencia']).attr("onclick",
                  "mostrarMensajeConfirmacion(\"eliminar verificacion laboral\"," + idVerificacion + "," + (
                    valor2['numero_referencia'] - 1) + ")");
                index++;
        ...
        success: function(res) {
          ...
          if (res != 0) {
            ...
            for (let number = 0; number < 1; number++) {
              $('#rowHistorialLaboral').append(
                '<div class="col-12 mt-5"><div class="text-center" id="topLaboral' + (number + 1) +
                '"><h4 class="text-primary"><strong>Laboral #' + (number + 1) + '</strong></h4><br></div></div>'
                );
              for (let tag of dato) {
                //* Boton por autor del registro del campo
                if (autor_anterior != '' && tag['autor'] != autor_anterior) {
        ...
        success: function(res) {
          ...
          if (res != 0) {
            ...
            for (let number = 0; number < 30; number++) {
              ...
              for (let tag of dato) {
                //* Opciones Select
                if (tag['referencia'] == 'trabajo_calidad' || tag['referencia'] == 'trabajo_puntualidad' || tag[
                    'referencia'] == 'trabajo_honesto' || tag['referencia'] == 'trabajo_responsabilidad' || tag[
                    'referencia'] == 'trabajo_adaptacion' || tag['referencia'] == 'trabajo_actitud_jefes' || tag[
                    'referencia'] == 'trabajo_actitud_companeros') {
                  opciones =
              ...
              $('#rowHistorialLaboral').append(
                '<div class="col-12"></div><div class="col-6 mt-3 mb-5"><button type="button" id="btnGuardarLaboral' +
                (number + 1) + '" class="btn btn-success btn-block" onclick="guardarLaboral(0,' + number + ',' +
                id + ',\'analista\',\'' + candidato + '\')">Guardar Laboral #' + (number + 1) +
                '</button></div><div class="col-6 mt-3 mb-5"><button type="button" id="btnEliminarLaboral' + (
                  number + 1) + '" class="btn btn-danger btn-block" disabled>Eliminar Laboral #' + (number +
                1) + '</button></div>');
            }
            ...
            if (valores != 0) {
              ...
              for (let valor of valores) {
                flag = 0;
                for (let tag of dato) {
                  $('[name="' + tag['atr_id'] + '[]"]').eq((valor['numero_referencia'] - 1)).val(valor[tag[
                    'referencia']]);
                  if (flag == 0) {
                    idLaboral = valor['id'];
                    flag++;
                ...
                $('#btnGuardarLaboral' + valor['numero_referencia']).attr("onclick", "guardarLaboral(" +
                  idLaboral + "," + (valor['numero_referencia'] - 1) + "," + id + ",\"analista\",\"" +
                  candidato + "\")");
                $('#btnEliminarLaboral' + valor['numero_referencia']).removeAttr('disabled');
                $('#btnEliminarLaboral' + valor['numero_referencia']).attr("onclick",
                  "mostrarMensajeConfirmacion(\"eliminar antecedente laboral\"," + idLaboral + "," + valor[
                    'numero_referencia'] + ")");
                index++;
              }
            }
          ...
          if (res != 0) {
            ...
            if (valores2 != 0) {
              ...
              for (let valor2 of valores2) {
                ...
                $('#btnGuardarVerificacion' + (index + 1)).attr("onclick", "guardarLaboral(" + idVerificacion +
                  "," + index + "," + id + ",\"analista\",\"" + candidato + "\")");
                $('#btnEliminarVerificacion' + valor2['numero_referencia']).removeAttr('disabled');
                $('#btnEliminarVerificacion' + valor2['numero_referencia']).attr("onclick",
                  "mostrarMensajeConfirmacion(\"eliminar contacto laboral\"," + idVerificacion + "," + valor2[
                    'numero_referencia'] + ")");
                index++;
              }
            }
    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        if (res != 0) {
          ...
          for (let number = 0; number < 30; number++) {
            $('#rowHistorialLaboral').append('<div class="col-12 mt-5"><div class="text-center" id="topLaboral' +
              (number + 1) + '"><h3 class="text-primary"><strong>Laboral #' + (number + 1) +
              '</strong></h3><br></div></div>');
            for (let tag of dato) {
              //* Opciones Select
              //* HTML
              if (tag['tipo_etiqueta'] == 'select') {
            ...
            $('#rowHistorialLaboral').append(
              ...
              (number + 1) + '" class="btn btn-success btn-block" onclick="guardarRefCliente(0,' + number +
              ',' + id + ',\'analista\',\'' + candidato + '\')">Guardar Referencia #' + (number + 1) +
              '</button></div><div class="col-6 mt-3 mb-5"><button type="button" id="btnEliminarCliente' + (
                number + 1) + '" class="btn btn-danger btn-block" disabled>Eliminar Referencia #' + (number +
              1) + '</button></div>');
          }
          //* Valores de laboral por el candidato
          if (valores != 0) {
            ...
            for (let valor of valores) {
              ...
              $('#btnGuardarCliente' + (index + 1)).attr("onclick", "guardarRefCliente(" + idLaboral + "," +
                index + "," + id + ",\"analista\",\"" + candidato + "\")");
              $('#btnEliminarCliente' + valor['numero_referencia']).removeAttr('disabled');
              $('#btnEliminarCliente' + valor['numero_referencia']).attr("onclick",
                "mostrarMensajeConfirmacion(\"eliminar referencia cliente\"," + idLaboral + "," + valor[
                  'numero_referencia'] + ")");
              index++;
            }
          }
    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        if (res != 0) {
          ...
          for (let number = 0; number < 1; number++) {
            $('#rowHistorialLaboral').append('<div class="col-12 mt-5"><div class="text-center" id="topLaboral' +
              (number + 1) + '"><h4 class="text-primary"><strong>Referencia #' + (number + 1) +
              '</strong></h4><br></div></div>');
            for (let tag of dato) {
              //* HTML
              if (tag['tipo_etiqueta'] == 'select') {
                $('#rowHistorialLaboral').append(tag['titulo_seccion_modal'] + tag['grid_col_inicio'] + tag[
    ...
    success: function(res) {
      ...
      if (res != 0) {
        ...
        for (let row of datos) {
          ...
          rows += '<tr id="fecha_inicio_gap' + row['id'] + '"><td>' + consecutivo + '</td><td>' + row[
              'fecha_inicio'] + '</td><td>' + row['fecha_fin'] + '</td><td>' + row['razon'] +
            '</td><td><a href="javascript:void(0)" class="btn btn-danger btn-sm" onclick="mostrarMensajeConfirmacion(\'eliminar gap\', ' +
            row['id'] + ', ' + id_candidato + ')"><i class="fas fa-trash"></i> Delete Gap</a></td></tr>'
          consecutivo++
        }
...
// Función para agregar candidatos al div flexible
function agregarCandidatoAlDivFlexible(candidatoId, candidatoText) {
  // Verificar si el candidato ya está presente en el div flexible
  if ($('#espacio_agregar_Candidatos').find('[data-id="' + candidatoId + '"]').length === 0) {
    // Si el candidato no está presente, agregarlo al div flexible
    var candidatoElement = $('<div class="candidato" data-id="' + candidatoId + '">ID: ' + candidatoId + ' - ' +
      candidatoText.split(' - ')[1] + '<button class="eliminarCandidato" data-id="' + candidatoId +
      '">x</button></div>');
    $('#espacio_agregar_Candidatos').append(candidatoElement);
  }
}

// Cargar candidatos al abrir el modal
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      $.each(response, function(index, candidato) {
        $('#getCandidatosFACIS').append($('<option>', {
          value: candidato.id,
          text: candidato.id + ' - ' + candidato.nombre + ' ' + candidato.paterno + ' ' + candidato
            .materno
        }));
      });
      ...
      $('#getCandidatosFACIS').select2({
        ...
        var candidatoId = e.params.data.id;
        var candidatoText = e.params.data.text;
        agregarCandidatoAlDivFlexible(candidatoId, candidatoText);
      });
    },
...
// Eliminar candidato en el div flexible
$('#espacio_agregar_Candidatos').on('click', '.eliminarCandidato', function(event) {
...
$('#ModalFACIS').on('hidden.bs.modal', function(e) {
  limpiarModalFACIS(); // Llama a la función para limpiar las fechas y el div flexible
});


function mostrarMensajeEmergente(mensaje, tipo) {
  var mensajeResultado = $('#mensajeResultado');
  mensajeResultado.removeClass('hidden alert-success alert-danger alert-warning');

  if (tipo === 'success') {
    mensajeResultado.addClass('alert-success');
  } else if (tipo === 'danger') {
    mensajeResultado.addClass('alert-danger');
  } else {
    mensajeResultado.addClass('alert-warning');
  }

  mensajeResultado.text(mensaje);
  mensajeResultado.removeClass('hidden');
}
...
function validarCampos() {
  ...
  var candidatos = $('#espacio_agregar_Candidatos .candidato').length;

  if (fecha_Inicio && fecha_Final && candidatos > 0) {
    $('#mensajeResultado').addClass('hidden');
    return true;
  } else {
    mostrarMensajeEmergente('Por favor, complete todos los campos y seleccione al menos un candidato.', 'danger');
    return false;
  }
}

$('#fecha_Inicio, #fecha_Final').on('change', validarCampos);
$('#espacio_agregar_Candidatos').on('DOMNodeInserted DOMNodeRemoved', validarCampos);

/****************************Función para actualizar FACIS********************/

function actualizarFacis() {
  if (!validarCampos()) {
    return;
  }

  var candidatos = [];
  $('#espacio_agregar_Candidatos .candidato').each(function() {
    candidatos.push($(this).find('button').data('id'));
  });
  ...
  $.ajax({
    url: '<?php echo base_url('Cliente_General/actualizarCandidatosFACIS'); ?>',
    type: 'POST',
    data: {
    ...
    error: function(xhr, status, error) {
      mostrarMensajeEmergente('Error en la solicitud.', 'danger');
    }
  });
}
