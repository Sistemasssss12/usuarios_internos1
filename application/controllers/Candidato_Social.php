...
class Candidato_Social extends CI_Controller{
  ...
  function getById(){
    $res = $this->candidato_social_model->getById($this->input->post('id'));
    if($res != null){
  ...
  function set(){
    ...
    if($data['campos']){
      ...
      if($fail > 0){
        ...
        $hayId = $this->candidato_social_model->countByIdCandidato($id_candidato);
        if($hayId == 0){
          $creacion = array(
            'creacion' => $date,
            'edicion' => $date,
          ...
          $this->candidato_social_model->add($creacion);
          foreach($data['campos'] as $c){
            ...
            $this->candidato_social_model->edit($edicion, $id_candidato);
          }
        }
        else{
          foreach($data['campos'] as $c){
        ...
        $msj = array(
          'codigo' => 1,
          'msg' => 'Antecedentes sociales actualizados correctamente'
        );
      }
    }
    else{
