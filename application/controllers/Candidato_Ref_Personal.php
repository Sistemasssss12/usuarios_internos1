...
class Candidato_Ref_Personal extends Custom_Controller{
  ...
  function getById(){
    $res = $this->candidato_ref_personal_model->getById($this->input->post('id'));
    if($res != null){
  ...
  function getComentarios(){
    $salida = "";
    $data['ref'] = $this->candidato_ref_personal_model->getComentarios($this->input->post('id'));
    if($data['ref']){
      foreach($data['ref'] as $ref){
        if($ref->comentario != null){
          ...
          if($ref->opinion_persona != null && $ref->opinion_trabajador != null){
            ...
            $salida .= 'sin opinión registrada de las referencias personales';
          }
        }
      }
      $res = trim($salida, ", ");
  ...
  function set(){
    ...
    if($data['campos']){
      ...
      if($fail > 0){
        ...
        $hayId = $this->candidato_ref_personal_model->countByIdCandidato($id_candidato);
        if($hayId == 0){
          $creacion = array(
            'creacion' => $date,
            'edicion' => $date,
          ...
          $id_ref = $this->candidato_ref_personal_model->add($creacion);
          foreach($data['campos'] as $c){
            ...
            $this->candidato_ref_personal_model->edit($edicion, $id_ref);
          }
          $mensaje = ($candidato->ingles == 0)? 'Referencia personal #'.$this->input->post('num').' actualizada correctamente' : 'Personal reference #'.$this->input->post('num').' updated successfully';
          $msj = array(
            'codigo' => 1,
            'msg' => $mensaje,
            'nuevo_id' => $id_ref
          ...
          if($this->input->post('id_ref') != 0){
            ...
            foreach($data['campos'] as $c){
              ...
              $this->candidato_ref_personal_model->edit($edicion, $this->input->post('id_ref'));
            }
            $mensaje = ($candidato->ingles == 0)? 'Referencia personal #'.$this->input->post('num').' actualizada correctamente' : 'Personal reference #'.$this->input->post('num').' updated successfully';
            $msj = array(
              'codigo' => 1,
            ...
            $mensajeAvance = ($candidato->ingles == 0)? '[System] Se han actualizado las referencias personales del candidato' : '[System] The candidate\'s personal references have been updated';
        ...
        if($this->session->userdata('tipo') == 3 && $msj['codigo'] == 1){
          $this->registrar_avance_candidato($id_usuario, 'personales');
        }
      }
    }
    else{
  ...
  function delete(){
    ...
    $this->candidato_ref_personal_model->delete($id);
    $mensaje = ($candidato->ingles == 0)? 'Referencia personal eliminada correctamente' : 'Personal reference deleted successfully';
    $msj = array(
      'codigo' => 1,
      'msg' => $mensaje
    );
