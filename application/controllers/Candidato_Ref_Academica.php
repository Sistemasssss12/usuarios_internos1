...
class Candidato_Ref_Academica extends Custom_Controller{
  ...
  function getById(){
    $res = $this->candidato_ref_academica_model->getById($this->input->post('id'));
    if($res != null){
  ...
  function set(){
    ...
    if($data['campos']){
      ...
      if($fail > 0){
        ...
        $hayId = $this->candidato_ref_academica_model->countByIdCandidato($id_candidato);
        if($hayId == 0){
          $creacion = array(
            'creacion' => $date,
            'edicion' => $date,
          ...
          foreach($data['campos'] as $c){
            ...
            $this->candidato_ref_academica_model->edit($edicion, $id_ref);
          }
          $mensaje = ($candidato->ingles == 0)? 'Referencia académica #'.$this->input->post('num').' actualizada correctamente' : 'Academic reference #'.$this->input->post('num').' updated successfully';
          $msj = array(
            'codigo' => 1,
            'msg' => $mensaje,
            'nuevo_id' => $id_ref
          ...
          if($this->input->post('id_ref') != 0){
            ...
            $mensajeAvance = ($candidato->ingles == 0)? '[System] Se han actualizado las referencias académicas del candidato' : '[System] The candidate\'s academic references have been updated';
        ...
        if($this->session->userdata('tipo') == 3 && $msj['codigo'] == 1){
          $this->registrar_avance_candidato($id_usuario, 'academicas');
        }
      }
    }
    else{
